...
from mr2.operators import SymmetrizedGradientOp
...
def test_symmetrized_gradient_op_forward(mode: Literal['central', 'forward', 'backward']) -> None:
    """Test symmetrized gradient of a simple linear vector field in 2D."""
    # Test vector field v = (v0, v1) in 2D
    size = 10
    ...
    dim = (-1, -2)
    symmetrized_gradient_op = SymmetrizedGradientOp(dim=dim, mode=mode)
    (symmetrized_gradient,) = symmetrized_gradient_op(v)

    # Verify correct values excluding borders
    sym_00 = symmetrized_gradient[0, 0, 1:-1, 1:-1]
    sym_11 = symmetrized_gradient[1, 1, 1:-1, 1:-1]
    sym_01 = symmetrized_gradient[0, 1, 1:-1, 1:-1]
    sym_10 = symmetrized_gradient[1, 0, 1:-1, 1:-1]
