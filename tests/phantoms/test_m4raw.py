...
def m4raw_data(tmp_path_factory):
    ...
    # XML header similar to the M4Raw dataset, containing only the elements used by M4RawDataset.
    # It is not valid ISMRMRD; for example, echo_train_length is a non-standard sequence parameter.
...
def test_m4raw_dataset_single_slice(m4raw_data):
    ...
    assert kdata.header.echo_train_length == 11
    torch.testing.assert_close(
        kdata.header.acq_info.idx.k1,
        torch.arange(30, 225)[None, None, None, :, None],
    )
    torch.testing.assert_close(
        kdata.header.acq_info.idx.repetition,
        torch.arange(N_REPS)[:, None, None, None, None],
    )
