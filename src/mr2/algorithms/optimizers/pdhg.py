...
from mr2.operators.functionals import ZeroFunctional
...
def pdhg(
    ...
    The operator can be supplied as a `~mr2.operators.LinearOperator` or as a
    :math:`m\times n` -`~mr2.operators.LinearOperatorMatrix`
    (you can use ``A | B`` for horizontal, ``A % B`` for vertical stacking).
    :math:`f` and :math:`g` can be single functionals or a `~mr2.operators.ProximableFunctionalSeparableSum` of m or n
