...
from mr2.operators.Operator import Operator, OperatorComposition, OperatorSum, Tin2
...
class LinearOperator(Operator[torch.Tensor, tuple[torch.Tensor]]):
    ...
    def __matmul__(
        ...
        """
        if isinstance(other, mr2.operators.IdentityOp):
            # neutral element of composition
            return self
        elif isinstance(self, mr2.operators.IdentityOp):
            return other
        elif isinstance(self, mr2.operators.ZeroOp) or isinstance(other, mr2.operators.ZeroOp):
            # zero operator composition with any operator is zero
            return mr2.operators.ZeroOp()
    ...
    def __add__(
        ...
        if isinstance(other, torch.Tensor | complex | int | float):
            # tensor addition
            return LinearOperatorSum(self, mr2.operators.IdentityOp() * other)
        elif isinstance(self, mr2.operators.ZeroOp):
    ...
    @overload
    def __or__(
        self, other: LinearOperator | mr2.operators.LinearOperatorMatrix
    ...
    def __or__(
        ...
        | mr2.operators.LinearOperatorMatrix
        | mr2.operators.OperatorStack,
    ) -> mr2.operators.LinearOperatorMatrix | mr2.operators.OperatorStack:
        ...
        if isinstance(other, mr2.operators.LinearOperatorMatrix):
            ...
            return mr2.operators.LinearOperatorMatrix([[self, other]])
        elif isinstance(other, mr2.operators.OperatorStack):
            if (rows := other.shape[0]) > 1:
                raise ValueError(
                    f'Shape mismatch in horizontal stacking: cannot stack LinearOperator and stack with {rows} rows.'
                )
            return mr2.operators.OperatorStack([[self, *other._operators[0]]])
        elif isinstance(other, Operator):
            return mr2.operators.OperatorStack(
                [[self, cast(Operator[torch.Tensor, tuple[torch.Tensor]], other)]]
            )
        return NotImplemented
    ...
    def __mod__(
        ...
        if isinstance(other, mr2.operators.LinearOperatorMatrix):
            ...
            if (cols := other.shape[1]) > 1:
                raise ValueError(
                    f'Shape mismatch in vertical stacking: cannot stack LinearOperator and stack with {cols} columns.'
                )
