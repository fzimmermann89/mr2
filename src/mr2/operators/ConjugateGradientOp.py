...
class ConjugateGradientOp(Operator[Unpack[tuple[torch.Tensor, ...]], tuple[torch.Tensor, ...]]):
    ...
    def __call__(
        ...
        initial_value
            A constant warm start for the conjugate gradient method. Elements requiring gradients are detached with a
            warning. If `None`, the initial value is set to zero.
    ...
    def forward(
        ...
        *parameters: torch.Tensor,
        ...
        if initial_value is not None and any(value.requires_grad for value in initial_value):
            warnings.warn(
                'Differentiable initial_value tensors are detached because initial_value is a constant warm start.',
                stacklevel=2,
            )
            initial_value = tuple(value.detach() for value in initial_value)
