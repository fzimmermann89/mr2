...
def collate_fn(batch: Any):  # noqa: ANN401
    ...
    return torch.utils.data._utils.collate.collate(
        batch,
        collate_fn_map={
            mr2.data.Dataclass: lambda batch, *, collate_fn_map: batch[0].stack(*batch[1:]),  # noqa: ARG005
...
class PINQI(torch.nn.Module):
    ...
    def get_linear_solver(self, gram: mr2.operators.LinearOperator):
    ...
    def forward(self, kdata: mr2.data.KData, csm: mr2.data.CsmData):
        ...
        images = list(mr2.algorithms.optimizers.cg(gram, zero_filled_image, max_iterations=2))
...
class PinqiModule(pl.LightningModule):
    ...
    def result_plot(
        ...
        pred: torch.Tensor,
        mask: torch.Tensor,
        axes: Sequence[plt.Axes] | np.ndarray,
