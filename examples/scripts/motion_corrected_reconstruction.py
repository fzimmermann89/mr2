...
kdata = mr2.data.KData.from_file(
    ...
    trajectory=mr2.data.traj_calculators.KTrajectoryRpe(angle=torch.pi * 0.618034),
)
...
def get_respiratory_self_navigator_from_grpe(
    kdata: mr2.data.KData, respiratory_frequency_range: tuple[float, float] = (0.2, 0.5)
) -> torch.Tensor:
    ...
    # Apply a 1D FFT along the readout to get the projection of the object
    fft_op_1d = mr2.operators.FastFourierOp(dim=(-1,))
...
# a look at `~mr2.algorithms.reconstruction.TotalVariationRegularizedReconstruction` to find out more.
#
...
motion_op = mr2.operators.GridSamplingOp.from_displacement(mf[..., 2], mf[..., 1], mf[..., 0])
...
#
# One way to solve this problem with MR2 is to use the respiratory-resolved k-space data from above. Because we used a
...
csm_op = mr2.operators.SensitivityOp(csm_maps)
averaging_op = mr2.operators.AveragingOp(dim=0)
...
(img_mcir,) = mr2.algorithms.optimizers.cg(
    operator,
    right_hand_side,
