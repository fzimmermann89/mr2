...
class AcceleratedFastMRI(torch.utils.data.Dataset):
    ...
    def __init__(self, path: Path, acceleration: float = 12, noise_level: float = 0.1):
        ...
        self.dataset = mr2.phantoms.FastMRIKDataDataset(files)
    ...
    def __getitem__(self, index: int) -> BatchType:
        ...
        csm = mr2.data.CsmData.from_kdata_inati(data, downsampled_size=64)
        target = mr2.algorithms.reconstruction.DirectReconstruction(data, csm=csm)(data)
        ...
        undersampling_mask = mr2.utils.pad_or_crop(random > threshold, data.data.shape[-2:])
        data_undersampled = data[..., undersampling_mask].rearrange('k ... 1 -> ... k')

        noise = mr2.utils.RandomGenerator(seed=index).randn_like(data_undersampled.data)
...
class MODL(torch.nn.Module):
    ...
    def __init__(self, iterations: int = 8, n_features: Sequence[int] = (64, 64, 64, 64)):
        ...
        network = mr2.nn.Residual(mr2.nn.ComplexAsChannel(mr2.nn.PermutedBlock((-1, -2), cnn)))
        self.network = cast(mr2.nn.Residual, torch.compile(network, dynamic=True, fullgraph=True))
        self.iterations = iterations
        self.regularization_weights = torch.nn.Parameter(0.2 * torch.ones(iterations))

    def __call__(self, kdata: mr2.data.KData, csm: mr2.data.CsmData) -> mr2.data.IData:
        """Apply MODL network.
    ...
    def forward(self, kdata: mr2.data.KData, csm: mr2.data.CsmData) -> mr2.data.IData:
        ...
        data_consistency_op = mr2.operators.ConjugateGradientOp(
            operator_factory=lambda _image, weight: gram + weight,
            rhs_factory=lambda image, weight: zero_filled_image + weight * image,
        )

        (image,) = mr2.algorithms.optimizers.cg(gram, zero_filled_image, max_iterations=5)
        for iteration in range(self.iterations):
            regularization = self.network(image)
            (image,) = data_consistency_op(regularization, self.regularization_weights[iteration])

        return mr2.data.IData(image, header=mr2.data.IHeader.from_kheader(kdata.header))


def plot(batch: BatchType, prediction: mr2.data.IData, step: int) -> None:
    """Plot the direct, sense, and modl reconstructions."""
    target = batch['target'].rss().cpu().squeeze()
    direct = (
        mr2.algorithms.reconstruction.DirectReconstruction(batch['data'], csm=batch['csm'])(batch['data'])
        .rss()
        .cpu()
        .squeeze()
    )
    ...
    sense = (
        mr2.algorithms.reconstruction.IterativeSENSEReconstruction(batch['data'], csm=batch['csm'])(batch['data'])
        .rss()
        .cpu()
        .squeeze()
    )
    ...
    ssim = mr2.operators.functionals.SSIM(mr2.utils.pad_or_crop(target[None], (320, 320)))

    def show(ax: matplotlib.axes.Axes, data: torch.Tensor, label: str):
        data = mr2.utils.pad_or_crop(data, (320, 320))
...
for i, batch in enumerate(pbar):
    ...
    objective = 0.5 * mr2.operators.functionals.MSE(target.data) - mr2.operators.functionals.SSIM(target.data)
